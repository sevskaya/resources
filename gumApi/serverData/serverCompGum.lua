if Config.Framework == "GUM" then
    ...
    function apiSrv.removeJob(identifier, charidentifier, jobId)
        exports.oxmysql:execute('SELECT job FROM characters WHERE identifier = ? AND charidentifier = ?', {identifier, charidentifier}, function(result)
            if result[1] ~= nil then
                local jobs = json.decode(result[1].job)
                for a,b in pairs(jobs) do
                    if b.job == jobId then
                        table.remove(jobs, a)
                    end
                end
                exports.oxmysql:execute('UPDATE characters SET job = ? WHERE identifier = ? AND charidentifier = ?', {json.encode(jobs), identifier, charidentifier})
            end
        end)
    end
    ...
    function apiSrv.getInformation(source, cb)
        ...
        if User then
            ...
            if Character ~= nil then
                ...
                player.setGrade = function(job, amount)
                    Character.setGrade(_source, job, grade)
                end
                ...
                player.chanceSkin = function(skin)
                    Character.buyClothes(_source, nil, json.decode(skin))
                end
