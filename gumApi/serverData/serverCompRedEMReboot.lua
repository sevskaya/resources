if Config.Framework == "REDEMREBOOT" then
    ...
    function apiSrv.setGrade(identifier, charidentifier, jobId, jobGrade)
        exports.oxmysql:execute('UPDATE characters SET jobgrade = ? WHERE identifier = ? AND characterid = ?', {jobGrade, identifier, charidentifier})
    end

    function apiSrv.removeJob(identifier, charidentifier, jobId)
        exports.oxmysql:execute('UPDATE characters SET job = ?, jobgrade = ?  WHERE identifier = ? AND characterid = ?', {"unemployed", 0, identifier, charidentifier})
    end
